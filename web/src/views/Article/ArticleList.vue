<template>
    <div class="container article-list">
        ...
        <section class="list">
                <div 
                    class="year-list" 
                    v-for="(val, key, idx) in requestDatas" 
                    :key="idx" 
                >
                    <ul 
                        class="month-list" 
  ...
  methods: {
    async getArticleList() {
      ...
      if (this.page > total) {
        ...
        //this.requestDatas = data
        for(let year in data){
         if(data.hasOwnProperty(year) && (!this.requestDatas.hasOwnProperty(year) )){
            this.requestDatas[year]=data[year];
    } else {
      for(let mon in data[year]){
         if(data[year].hasOwnProperty(mon) && (!this.requestDatas[year].hasOwnProperty(mon) )){
            this.requestDatas[year][mon]=data[year][mon];
      } else {
        //Object.assign(this.requestDatas[year][mon], data[year][mon]);
      }
    }
        }
